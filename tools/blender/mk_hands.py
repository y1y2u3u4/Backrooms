...
def build_one_hand(side_sign, mat_skin, mat_sleeve):
    ...
    thenar = A.prim_sphere("_thenar", 0.022, segments=8, rings=5,
    ...
    for fi, (fx, flen, r0, r1, curls) in enumerate(finger_specs):
        ...
        for si, seg_len in enumerate(seg_lens):
            ...
            rr0 = r0 + (r1 - r0) * (si / 3)
            rr1 = r0 + (r1 - r0) * ((si + 1) / 3)
            seg = A.cylinder_between(f"_f{fi}s{si}", pos, end, rr0, rr1, segments=5)
            parts.append((seg, mat_skin))
            # small knuckle bead at each joint to hide the seam
            bead = A.prim_sphere(f"_f{fi}b{si}", rr0 * 1.05, segments=5, rings=4, location=tuple(pos))
            parts.append((bead, mat_skin))
            pos = end
        tip = A.prim_sphere(f"_f{fi}tip", r1, segments=5, rings=4, location=tuple(pos))
    ...
    t0 = A.cylinder_between("_thumb0", thumb_root, thumb_mid, 0.0125, 0.0098, segments=5)
    t1 = A.cylinder_between("_thumb1", thumb_mid, thumb_tip, 0.0098, 0.0072, segments=5)
    t_base_bead = A.prim_sphere("_thumbBase", 0.0135, segments=5, rings=4, location=tuple(thumb_root))
    t_knuckle = A.prim_sphere("_thumbKnuckle", 0.0098, segments=5, rings=4, location=tuple(thumb_mid))
    t_tip = A.prim_sphere("_thumbTip", 0.0072, segments=5, rings=4, location=tuple(thumb_tip))
